...
Step 3:
--------
Add views for all action methods razor view only as i need the layout and dont select any template just add view add and add the code which i had given below Note that for logout view is not needed dont generate it i will add one link in Layout  view of  when u click that u will jump to Login Page view only so 
